...
{
    protected \GuzzleHttp\Client $naiveHttpClient;
    ...
    private string $_type = "SDK";

    private string $_version = "php:4.1.12";
...
PUBLICKKEY;
    ...
    {
        $result = $this->send($this->host . $path, $method = 'GET');
        $res = json_decode(json_encode($result, JSON_THROW_ON_ERROR), null, 512, JSON_THROW_ON_ERROR);
        return $res->data ?? $res;
    ...
    {
        $result = $this->send($this->host . $path, $method = 'DELETE');
        $res = json_decode(json_encode($result, JSON_THROW_ON_ERROR), null, 512, JSON_THROW_ON_ERROR);
        return $res->data ?? $res;
        // return $this->arrayToObject($result);
    }
     ...
     * @throws Exception
     */
    private function send($url, string|array $data = '', $method = 'POST', $time = 30000)
    {
        $token = $this->getToken();
        // 如果是通过密钥刷新
